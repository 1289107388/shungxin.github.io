...
SELECT
  ...
  COALESCE(SUM(il.count), 0)::bigint AS total_likes,
  COALESCE(SUM(iv.count), 0)::bigint AS total_views
FROM public.users u
LEFT JOIN public.gallery_images gi
  ON gi.uploaded_by::bigint = u.id AND gi.is_visible = true
